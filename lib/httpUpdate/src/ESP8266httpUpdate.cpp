...
// const uint8_t fingerprint[20] = {0x84, 0x63, 0xB3, 0xA9, 0x29, 0x12, 0xCC, 0xFD, 0x1D, 0x31, 0x47, 0x05, 0x98, 0x9B, 0xEC, 0x13, 0x99, 0x37, 0xD0, 0xD7};
const char *fingerprint = "84 63 B3 A9 29 12 CC FD 1D 31 47 05 98 9B EC 13 99 37 D0 D7";
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    http.begin(client, url); //, httpsFingerprint
    // http.begin(*client, url);
#pragma GCC diagnostic pop
    return handleUpdate(http, currentVersion, false);
}
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    http.begin(client, host, port, url, httpsFingerprint);
    // http.begin(*client, host, port, url, httpsFingerprint);
#pragma GCC diagnostic pop
    return handleUpdate(http, currentVersion, false);
}
    ...
    case HTTP_CODE_OK: ///< OK (Start Update)
        if (len > 0)
        {
            ...
            else
            {
                ...
                WiFiClientSecure *_owner;
                if (_closeConnectionsOnUpdate)
                {
                    WiFiUDP::stopAll();
                    // WiFiClient::stopAllExcept(_owner);
                }
                ...
                if (!spiffs)
                {
                    ...
                    while (i < 16)
                    {
                        uint8_t payload = client;
                        buf[0] = payload;
                        // Serial.print(payload);
                        //  buf[i] = client->readBytes(buf_c, buf_len);
                        // Serial.print(buf[i]);
                        i++;
                    }
                ...
                if (runUpdate(client, len, http.header("x-MD5"), command))
                {
                    ret = HTTP_UPDATE_OK;
